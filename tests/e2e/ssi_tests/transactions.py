...
def form_did_create_tx_multisig(diddoc, signPairs, blockchain_account):
    ...
    cmd_string = f"hid-noded tx ssi register-did '{json.dumps(diddoc)}' {proofsStr} --from {blockchain_account} " + COMMON_TX_COMMAND_FLAGS
    return cmd_string
...
def form_did_update_tx_multisig(diddoc, signPairs, blockchain_account):
    ...
    cmd_string = f"hid-noded tx ssi update-did '{json.dumps(diddoc)}' '{version_id}' {proofsStr} --from {blockchain_account} " + COMMON_TX_COMMAND_FLAGS
    return cmd_string
...
def form_did_deactivate_tx_multisig(didId, signPairs, blockchain_account):
    ...
    cmd_string = f"hid-noded tx ssi deactivate-did '{didId}' '{version_id}' {proofsStr} --from {blockchain_account} " + COMMON_TX_COMMAND_FLAGS
    return cmd_string
...
def form_update_schema_tx(schema_msg, schema_proof, blockchain_account):
    cmd_string = f"hid-noded tx ssi update-schema '{json.dumps(schema_msg)}' '{json.dumps(schema_proof)}' --from {blockchain_account} " + COMMON_TX_COMMAND_FLAGS
    return cmd_string
...
def form_update_cred_status_tx(cred_msg, cred_proof, blockchain_account):
    cmd_string = f"hid-noded tx ssi update-credential-status '{json.dumps(cred_msg)}' '{json.dumps(cred_proof)}' --from {blockchain_account} " + COMMON_TX_COMMAND_FLAGS
    return cmd_string

...
def form_did_create_tx(did_doc, kp, blockchain_account, verificationMethodId=None):
    private_key = kp["priv_key_base_64"]
    if not verificationMethodId:
        verificationMethodId = did_doc["authentication"][0]
    
    cmd_string = f"hid-noded tx ssi create-did '{json.dumps(did_doc)}' {verificationMethodId} --ver-key {private_key} --from {blockchain_account} " + COMMON_TX_COMMAND_FLAGS
    return cmd_string
...
def form_did_update_tx(did_doc, kp, blockchain_account, verificationMethodId=None):
    private_key = kp["priv_key_base_64"]
    if not verificationMethodId:
        verificationMethodId = did_doc["authentication"][0]
    version_id = query_did(did_doc["id"])["didDocumentMetadata"]["versionId"]
    cmd_string = f"hid-noded tx ssi update-did '{json.dumps(did_doc)}' '{version_id}' '{verificationMethodId}' --from {blockchain_account} --ver-key {private_key} " + COMMON_TX_COMMAND_FLAGS
    return cmd_string

def form_did_deactivate_tx(did_doc_id, kp, blockchain_account, verificationMethodId=None):
    private_key = kp["priv_key_base_64"]
    if not verificationMethodId:
    ...
    cmd_string = f"hid-noded tx ssi deactivate-did '{did_doc_id}' '{version_id}' '{verificationMethodId}' --from {blockchain_account} --ver-key {private_key} " + COMMON_TX_COMMAND_FLAGS
    return cmd_string

...
func EdDSACryptoSuite2020Canonize(didDoc *types.DidDocument) ([]byte, error) {
...
func EcdsaSecp256k1RecoverySignature2020Canonize(didDoc *types.DidDocument) ([]byte, error) {
	jsonLdDid := NewJsonLdDid(didDoc)
	canonizedDidDocument, err := jsonLdDid.NormalizeWithURDNA2015()
	if err != nil {
		return nil, err
...
func BbsBlsSignature2020Canonize(didDoc *types.DidDocument) ([]byte, error) {
	jsonLdDid := NewJsonLdDid(didDoc)
	canonizedDidDocument, err := jsonLdDid.NormalizeWithURDNA2015()
	if err != nil {
		return nil, err
...
func EcdsaSecp256k1Signature2019Canonize(didDoc *types.DidDocument) ([]byte, error) {
	jsonLdDid := NewJsonLdDid(didDoc)
	canonizedDidDocument, err := jsonLdDid.NormalizeWithURDNA2015()
	if err != nil {
		return nil, err
